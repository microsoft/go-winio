...
// TODO(thaJeztah): the code below should be generated by mkwinsyscall (and probably needs a 32-bit wrapper)
var procEventWriteString = modadvapi32.NewProc("EventWriteString")

// WriteEventString writes a single ETW event from the provider.
func (provider *Provider) WriteEventString(message string) error {
	...
	r0, _, _ := procEventWriteString.Call(uintptr(provider.handle), 0, 0, uintptr(unsafe.Pointer(&utf16message[0])))
	if r0 != 0 {
